...
const proto = Object.create(viewBase.base.prototype);
let mapObj;
...
proto.create = function (map, initiative, config) {
  ...
  if (!initiative.hasLocation()) {
    const classes = ['awesome-marker', 'sea-non-geo-marker']
      .concat(classesForFields(initiative, this.config.fields()));
...
function init(registry) {
  ...
  function createMarker(map, initiative) {
    ...
    view.create(map, initiative, config);
    return view;
  }
