...
Generates CSV datasets for testing mykomap.


## USAGE

    ts-node gen-csv.ts [N]

Where:

- N: number of rows to generate. If omitted, only one row is emitted.

Results are in CSV format, with a leading header row, printed on standard output.
...
Currently outputs only the "standard.csv" headers, setting only
Identifier, Name, and location.
...
type ValueGenerator = (ix: number) => string;
...
const Empty: ValueGenerator = _ => '';

const Id: ValueGenerator = ix => `id${ix+1}`;

const Label: ValueGenerator = ix => upcaseFirst(randomTerms(ix, 3));

const Phrase: ValueGenerator = ix => upcaseFirst(randomTerms(ix, 1)) + " " + randomTerms(ix+1, 20);

function Category(values: string[]): ValueGenerator {
  return ix => randomTerms(ix, 1, values);
}

// Returns a fixed value
function Fixed(value: string): ValueGenerator {
  return _ => value;
}

// Returns a floating point number between the two ranges
function FloatRange(lower: number, upper: number): ValueGenerator {
  return ix => String(prng()*(upper - lower)+lower);
}
...
class Generator {
  ...
  generate(numItems: number): void {
    // Outut the headings
    console.log(Object.keys(this.schema).map(this.escape).join(','));

    const generators = Object.values(this.schema);

    // Output the rows
    for(let ix = 0; ix < numItems; ix++) {
      const row = generators.map(gen => this.escape(gen(ix))).join(",");
      console.log(row);
    }
  }
...
// Generate something
new Generator(stdHeaders4).generate(rows);
